...
// vetor que guarda posições do evento (x,y,tipo)
const pointerPositions = [];
const MAX_POINTER_POSITIONS = 200; // limite para evitar uso excessivo de memória
...
// ======= INPUTS: listeners registrados UMA vez, empurram posições para pointerPositions =======
function setupInput() {
    if (!canvas) return;
    ...
    if (supportsPointer) {
        canvas.addEventListener("pointerdown", (e) => {
            // para touch: consideramos arraste (active)
            pushPointer(e.clientX, e.clientY, e.pointerType, true);
        });
        canvas.addEventListener("pointermove", (e) => {
            // pointermove: para mouse atualiza sempre; para touch atualiza enquanto arrastando (buttons or isPrimary)
            // alguns navegadores não setam buttons para touch, então aceitamos sempre pointermove para simplicidade
            pushPointer(e.clientX, e.clientY, e.pointerType, e.buttons !== 0 || e.pointerType === 'touch');
        });
        canvas.addEventListener("pointerup", (e) => {
            pushPointer(e.clientX, e.clientY, e.pointerType, false);
        });
        ...
        canvas.addEventListener("mousemove", (e) => {
            pushPointer(e.clientX, e.clientY, 'mouse', true);
        });
        canvas.addEventListener("mousedown", (e) => {
            pushPointer(e.clientX, e.clientY, 'mouse', true);
        });
        canvas.addEventListener("mouseup", (e) => {
            pushPointer(e.clientX, e.clientY, 'mouse', false);
        });

        canvas.addEventListener("touchstart", (e) => {
            const t = e.touches[0];
            if (t) pushPointer(t.clientX, t.clientY, 'touch', true);
            e.preventDefault();
        }, { passive: false });
        ...
        canvas.addEventListener("touchend", (e) => {
            // touchend não tem touches[0], usar changedTouches
            const t = e.changedTouches && e.changedTouches[0];
            if (t) pushPointer(t.clientX, t.clientY, 'touch', false);
        }, { passive: true });
    }
...
// empurra posição no vetor e mantém tamanho máximo
function pushPointer(x, y, type = 'mouse', active = true) {
    pointerPositions.push({ x: Math.max(0, Math.min(W, x)), y: Math.max(0, Math.min(H, y)), type, active, t: Date.now() });
    if (pointerPositions.length > MAX_POINTER_POSITIONS) pointerPositions.shift();
}

// pega a última posição válida do vetor (ou null)
function getLastPointer() {
    if (pointerPositions.length === 0) return null;
    return pointerPositions[pointerPositions.length - 1];
}

// ======= LOOP PRINCIPAL (lê pointerPositions a cada frame) =======
function loop() {
    if (!ctx) return;

    // --- leitura de input por frame (usa o vetor) ---
    const last = getLastPointer();
    if (last) {
        // comportamento solicitado:
        // - se iniciou no mobile (touch), considerar arraste (active true) para mover
        // - se desktop (mouse), considerar movimento do mouse (sempre atualiza)
        if (last.type === 'touch') {
            // para touch, só atualiza destino se o evento estiver ativo (arrastando)
            if (last.active) {
                destino.x = last.x;
                destino.y = last.y;
            }
        } else {
            // mouse ou pointerType diferente: atualiza sempre (movimento do mouse)
            destino.x = last.x;
            destino.y = last.y;
        }
    }
